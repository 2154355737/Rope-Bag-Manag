...
import { getResourceRecords, deleteResourceRecord } from '../../api/resourceRecords'
...
async function loadRecords() {
  ...
  try {
    ...
    const response = await getResourceRecords(params)
    if (response.code === 0) {
...
async function deleteRecord(record: any) {
  try {
    ...
    const response = await deleteResourceRecord(record.id)
    if (response.code === 0) {
