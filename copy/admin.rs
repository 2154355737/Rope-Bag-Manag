...
async fn update_mail_settings(
    ...
    match admin_service.update_mail_settings(&req.0).await {
        Ok(_) => {
            // 热更新邮件服务实例
            if let Ok(mail_config) = serde_json::from_value::<MailConfig>(req.0.clone()) {
                if let Ok(new_service) = EmailService::new(&mail_config) {
                    let mut es = email_service.write().await;
                    *es = new_service;
                }
            }
            Ok(HttpResponse::Ok().json(json!({
...
async fn send_test_email(
    ...
    match es.send(email, "【绳包管理器】测试邮件", "这是一封测试邮件，如果您收到此邮件，说明邮件服务配置正确。").await {
        Ok(_) => Ok(HttpResponse::Ok().json(json!({
            "code": 0,
            "message": "测试邮件发送成功"
        }))),
