...
impl AuthService {
    ...
    pub async fn register(&self, req: &CreateUserRequest) -> Result<LoginResponse> {
        // 用户名与昵称校验
        let username_re = regex::Regex::new(r"^[A-Za-z0-9_]{1,10}$").unwrap();
        if !username_re.is_match(&req.username) {
            return Err(anyhow::anyhow!("用户名仅允许字母、数字、下划线，且不超过10位"));
        }
        if let Some(nick) = &req.nickname {
            // 昵称：长度≤8，允许中文、英文、数字、下划线
            let nickname_re = regex::Regex::new(r"^[\p{Han}A-Za-z0-9_]{1,8}$").unwrap();
            if !nickname_re.is_match(nick) {
                return Err(anyhow::anyhow!("昵称仅允许中文、字母、数字、下划线，且不超过8位"));
            }
        }
        // 验证邮箱验证码
